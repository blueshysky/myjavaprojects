...
//create formatted object to write output directly to console & file
	...
	private static void AvgComp() {
		...
		int count = 0;
		for (int i = 0; i < robjs.length; i++) {
			if (robjs[i].getSex().equals("FEMALE") && robjs[i].getMortage().equals("YES")
		...
		;
		for (int i = 0; i < robjs.length; i++) {
			if (robjs[i].getSex().equals("MALE") && robjs[i].getRegion().contentEquals("INNER_CITY")
					&& robjs[i].getCar().equals("YES") && robjs[i].getChildren() == 1) {
				++inner;
			}
			if (robjs[i].getSex().equals("MALE") && robjs[i].getRegion().contentEquals("RURAL")
					&& robjs[i].getCar().equals("YES") && robjs[i].getChildren() == 1) {
				++rural;
			}

			if (robjs[i].getSex().equals("MALE") && robjs[i].getRegion().contentEquals("SUBURBAN")
					&& robjs[i].getCar().equals("YES") && robjs[i].getChildren() == 1) {
				++subarban;
			}

			if (robjs[i].getSex().equals("MALE") && robjs[i].getRegion().contentEquals("TOWN")
					&& robjs[i].getCar().equals("YES") && robjs[i].getChildren() == 1) {
				++town;
			}
		}
